...
This example is contributed from the https://www.smooks.org/[Smooks] community. As depicted below, it illustrates a common supply chain transaction flow where (1) a customer transmits an EDI purchase order, requesting goods or services, to a supplier and (2) the supplier acknowledges the transaction. Along the way, the supplier tracks the customer purchase order while also exchanging bits and pieces of the order with its internal systems.
...
2. From another terminal window, send an X12 850 purchase order to the AS2 Camel endpoint, or better still, dispatch the message from Camel JBang as shown next:
+
[source,shell]
----
